    ...
    def post(self, request, courseId, user):
        try:
            ...
            checkout_session = stripe.checkout.Session.create(
                ...
                payment_method_types=['card', ],
                mode='payment',
                success_url=f'https://chefcharisma.akbarali.shop/api/stripe/success-checkout/?success=true&session_id={{CHECKOUT_SESSION_ID}}&courseId={course.id}&user={user_instance.username}',
                cancel_url=settings.SITE_URL + '/?canceled=true',
            )
